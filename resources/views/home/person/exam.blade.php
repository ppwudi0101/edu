    ...
    <div class="course-problem-cont">
        ...
        <div class="subtitle-cont container">
            ...
            <div>
                ...
                @foreach($dan as $d)
                                <li class="problem-choice">{{$loop->iteration}}. {{$d->question_name}}&nbsp;&nbsp;（{{$d->question_score}}）分
                    <label><input type="radio" value="1" name="answer_[{{$d->id}}][]">
                        A:{{$d->option_a}}</label>
                    <label><input type="radio" value="2" name="answer_[{{$d->id}}][]">
                        B:{{$d->option_b}}</label>
                    <label><input type="radio" value="4" name="answer_[{{$d->id}}][]">
                        C:{{$d->option_c}}</label>
                    <label><input type="radio" value="8" name="answer_[{{$d->id}}][]">
                        D:{{$d->option_d}}</label>
                </li>
                @endforeach
                <div class="title">二、选择题：(多选)</div>
                @foreach($duo as $dd)
                                    <li class="problem-choice">{{$loop->iteration}}. {{$dd->question_name}}&nbsp;&nbsp;（{{$dd->question_score}}）分
                        <label><input type="checkbox" value="1" name="answer_[{{$dd->id}}][]">
                            A:{{$dd->option_a}}</label>
                        <label><input type="checkbox" value="2" name="answer_[{{$dd->id}}][]">
                            B:{{$dd->option_b}}</label>
                        <label><input type="checkbox" value="4" name="answer_[{{$dd->id}}][]">
                            C:{{$dd->option_c}}</label>
                        <label><input type="checkbox" value="8" name="answer_[{{$dd->id}}][]">
                            D:{{$dd->option_d}}</label>
                    </li>
                    @endforeach            
